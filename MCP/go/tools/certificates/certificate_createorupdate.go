...
func Certificate_createorupdateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CertificateCreateOrUpdateParameters
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/certificates/%s%s", cfg.BaseURL, resourceGroupName, serviceName, certificateId, subscriptionId, queryString)
		...
		var result models.CertificateContract
...
func CreateCertificate_createorupdateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_certificates_certificateId",
		mcp.WithDescription("Creates or updates the certificate being used for authentication with the backend."),
		...
		mcp.WithString("certificateId", mcp.Required(), mcp.Description("Identifier of the certificate entity. Must be unique in the current API Management service instance.")),
		mcp.WithString("If-Match", mcp.Description("The entity state (Etag) version of the certificate to update. A value of \"*\" can be used for If-Match to unconditionally apply the operation..")),
		...
		mcp.WithObject("properties", mcp.Description("Input parameter: Parameters supplied to the CreateOrUpdate certificate operation.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Certificate_createorupdateHandler(cfg),
	}
}
