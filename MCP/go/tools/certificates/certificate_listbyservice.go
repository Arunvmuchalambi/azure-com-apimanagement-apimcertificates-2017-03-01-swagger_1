...
func Certificate_listbyserviceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/certificates%s", cfg.BaseURL, resourceGroupName, serviceName, subscriptionId, queryString)
		...
		var result models.CertificateCollection
...
func CreateCertificate_listbyserviceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_certificates",
		mcp.WithDescription("Lists a collection of all certificates in the specified service instance."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Certificate_listbyserviceHandler(cfg),
	}
}
