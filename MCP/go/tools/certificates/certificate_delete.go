...
func Certificate_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCertificate_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_certificates_certificateId",
		mcp.WithDescription("Deletes specific certificate."),
		...
		mcp.WithString("If-Match", mcp.Required(), mcp.Description("The entity state (Etag) version of the certificate to delete. A value of \"*\" can be used for If-Match to unconditionally apply the operation.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Certificate_deleteHandler(cfg),
	}
}
